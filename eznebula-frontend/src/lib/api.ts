...
export const eznebulaApi = {
  joinNetwork: (req: JoinRequest) =>
    invoke<string>("join_network", { request: req }),

  disconnectNetwork: () =>
    invoke<void>("disconnect_network"),

  getStatus: () =>
    invoke<NetworkStatus>("get_status"),

  getNetworkStats: () =>
    invoke<NetworkStats>("get_network_stats"),

  saveServer: (name: string, address: string, port: number) =>
    invoke<ServerEntry>("save_server", { name, address, port }),

  getServers: () =>
    invoke<ServerEntry[]>("get_servers"),
